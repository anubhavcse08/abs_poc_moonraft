...
class TableDesc extends Component {
  ...
  renderTableHeaderCell = (headerTitle) => {
    return (
      <tr>
        <th className="py-2 border border-slate-300 bg-white font-semibold w-60 px-2 text-left">
          {headerTitle}
        </th>
        {this.state.foreCastDetails.map((item, index) => {
          return (
            <th
              key={index}
              className="py-2 border border-slate-300 bg-white font-semibold text-center w-20"
            >
              {headerTitle === "Weeks" ? (
                <>
  ...
  renderCellStatusLabel = (isStatusBar, week, item, dataCell, i, count) => {
    return (
      <>
        {isStatusBar && (
          <td
            className={`py-2 custom-height-${item.noOfRows} border border-slate-300 bg-white w-20 relative`}
            key={i}
          >
        ...
        <tr key={index}>
          <td className="py-2 border border-slate-300 bg-white font-semibold w-96 px-2">
            <div className="flex flex-row items-baseline">
              ...
              <p className="whitespace-no-wrap text-gray-900 text-sm ml-1">
                {item.Title}
              </p>
            </div>
          </td>
