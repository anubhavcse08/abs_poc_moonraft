...
class TableDesc extends Component {
  ...
  renderTableHeaderCell = (headerTitle) => {
    return (
      <tr>
        <th className="py-2 border border-slate-300 bg-white font-semibold w-60 px-2 text-left text-xs">
          {headerTitle}
        </th>
        {this.state.foreCastDetails.map((item, index) => {
          return (
            <th
              key={index}
              className={`${
                headerTitle === "Weeks" ? "py-1" : "py-3"
              } border border-slate-300 bg-white font-semibold text-center w-20`}
            >
              {headerTitle === "Weeks" ? (
                <>
                ...
                <>
                  ...
                  <p
                    className={`${this.getForecastStatus(
                      item.targetType
                    )} py-3 w-full`}
                  ></p>
                </>
  ...
  renderCellStatusLabel = (isStatusBar, week, item, dataCell, i, count) => {
    return (
      <>
        {isStatusBar && (
          <td
            className={`py-2 custom-height-${item.noOfRows} border border-x-slate-300 bg-white w-20 relative`}
            key={i}
          >
  ...
  renderTableData = () => {
    return this.state.weekWiseDetails.data.map((item, index) => {
      return (
        <tr key={index} className="border-dotted border-t-2 border-y-slate-300">
          <td className="py-2 border border-x-slate-300 bg-white font-semibold w-60 px-2">
            <div className="flex flex-row items-baseline">
              ...
              />
              <p className="whitespace-nowrap overflow-hidden text-ellipsis text-gray-900 text-xs font-semibold ml-1">
                {item.Title}
              </p>
            </div>
          </td>
