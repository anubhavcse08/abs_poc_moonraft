...
const getStatusColor = (status) => {
  ...
  switch (status) {
    ...
    case "Planning":
      ...
      borderColor = 'border-dotted border-l-2 border-emerald-500';
      break;
    case "Executed":
      bgColor = 'bg-blue-100';
...
const DataCellLabel = (props) => {
  ...
  return (
    <div className={`data-cell-label absolute z-10 ${row > 1 ? `top-cell-${row}` : 'top-0.5'} left-0.2 py-1 ${bgColor} flex flex-row flex-wrap justify-left items-center ${borderColor} text-custom-small cell-width-${weeks}`}>
